...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    $sql = "SELECT * FROM seriesgenres WHERE genre = '" . $genre . "' ORDER BY showname ASC";
    $genreresults = mysqli_query($conn, $sql);
    $count = mysqli_num_rows($genreresults);
}
?>
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <div class="topbar-filter fw">
                        ...
                        // $countresults = mysqi_query($conn, $sql);
                        // $count = mysqli_num_rows($countresults);
                        ?>
                        <p>Found <span><?php echo $count; ?> Tv Shows</span> in total</p>
                        <!-- <label>Sort by:</label>
                        <select>
                    ...
                    <div class="flex-wrap-movielist mv-grid-fw">
                        <?php
                        while ($shows = mysqli_fetch_assoc($genreresults)) {
                            $name = $shows['showname'];
                            $sql = "SELECT * FROM series WHERE seriesname = '" . $name . "'";
                            $seriesresult = mysqli_query($conn, $sql);
                            while ($shows = mysqli_fetch_assoc($seriesresult)) { ?>
                        <div class="movie-item-style-2 movie-item-style-1">
                          <a href="series.php?tv-show=<?php echo $shows['seriesname']; ?>">
                            <img class="lazy" data-src="http://dsconlimited.net/dsconlimitedseries/<?php echo $shows['folder'] ?>/<?php echo $shows['poster']; ?>" alt="">
                          </a>
                            <div class="mv-item-infor">
                                <h6><a href="series.php?tv-show=<?php echo $shows['seriesname']; ?>"><?php echo $shows['seriesname']; ?></a></h6>
                                <p class="rate"><i class="ion-android-star"></i><span><?php echo round($shows['vote_average']); ?></span> /10</p>
                            </div>
                        </div>
                            <?php }
                        } ?>
